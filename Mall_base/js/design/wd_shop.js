...
function pagerSlider(){
	if(!($('.area_shopView').length > 0)) return;		
...
function productList(){
	...
	$.each(shopArr, function(index, item){
		...
		var listImg = $(this).children().length;
...
function optionChage(){
	$('a[data-shop-layer="view"]').on('click',function(){
		$("*[data-shop-layer='pop'] .close").parent().hide();
...
function viewListControl(){
	if(!($('.list-control').length > 0)) return false;
	$('.list-control').each(function(){
		let parentHtml = $(this).parents('article, .default'),
			link = $(this).children('button'),
			contsHtml = parentHtml.find('.view-control');

		if(link.hasClass('active')){
			let attrData = $(this).data('listview');
			contsHtml.attr('data-viewnumber', attrData);
		}
